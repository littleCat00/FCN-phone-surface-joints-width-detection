...
from FCN_layers import deconv_layer
...
def inference(x0, is_training,
    ...
    #  全连接层换成卷积层
    with tf.variable_scope('fc2conv'):
        c['ksize'] = 7
        c['stride'] = 1
        c['conv_filters_out'] = num_classes
        convfc = conv(x,c)

    # upsampling inference
    # Deconv1 -  32x
    deconv1_shape = tf.shape(x4)
    deconv1_out_channels = x4.get_shape()[3].value
    deconv1 = deconv_layer('deconv1', convfc, 4, 4, deconv1_out_channels, num_classes,
                           deconv1_shape, 2, 2, 'SAME')
    fuse1 = tf.add(deconv1, x4, name='fuse1')

    # Deconv2 - 16x
    deconv2_shape = tf.shape(x3)
    deconv2_out_channels = x3.get_shape()[3].value
    deconv2 = deconv_layer('deconv2', fuse1, 4, 4, deconv2_out_channels, deconv1_out_channels,
                           deconv2_shape, 2, 2, 'SAME')
    fuse2 = tf.add(deconv2, x3, name='fuse2')

    # Deconv3 - 8x
    deconv3_shape = tf.shape(x0)
    deconv3_shape = tf.stack([deconv3_shape[0], deconv3_shape[1], deconv3_shape[2], num_classes])
    deconv3 = deconv_layer('deconv3', fuse2, 16, 16, num_classes, deconv2_out_channels,
